...
# Browsers
push @ALL_TESTS,(qw(mosaic netscape nav2 nav3 nav4 nav4up nav45 nav45up nav6 nav6up navgold firefox safari ie ie3 ie4 ie4up ie5 ie5up ie55 ie55up ie6 ie7 ie8 opera opera3 opera4 opera5 opera6 opera7 opera8 opera9 opera10 lynx links aol aol3 aol4 aol5 aol6 aol7 aol8 aol9 neoplanet neoplanet2 avantgo emacs mozilla gecko konqueror));

# Robots
push @ALL_TESTS,(qw(wget getright robot yahoo altavista lycos infoseek lwp webcrawler linkexchange slurp webtv staroffice lotusnotes icab google java));
...
sub _test {
 ...
 $tests->{NETSCAPE}  = (!$tests->{FIREFOX} && !$tests->{SAFARI} && index($ua,"mozilla") != -1 && 
  ...
  $tests->{AOL7}      = (index($ua,"aol 7.0") != -1);
  $tests->{AOL8}      = (index($ua,"aol 8.0") != -1);
  $tests->{AOL9}      = (index($ua,"aol 9.0") != -1);
  ...
  $tests->{OPERA7}    = (index($ua,"opera 7") != -1) || (index($ua,"opera/7") != -1);
  $tests->{OPERA8}    = (index($ua,"opera 8") != -1) || (index($ua,"opera/8") != -1);
  $tests->{OPERA9}    = (index($ua,"opera 9") != -1) || (index($ua,"opera/9") != -1);
  $tests->{OPERA10}    = (index($ua,"opera 10") != -1) || (index($ua,"opera/10") != -1);
