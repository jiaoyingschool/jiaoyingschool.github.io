...
sub check_for_iTransact_response {
  if ($form_data{'iTransact'} eq 'iTransact') {
...
&codehook("iTransact_fields_bottom");

}

###############################################################################
sub iTransact_order_form_prep { # load the customer info ...
  &iTransact_check_and_load;
...
&codehook("iTransact_table_setup_bottom");
...
$my_submit=&script_and_substitute($sc_itransact_submit);
$my_change=&script_and_substitute($sc_itransact_change);
$my_message=&script_and_substitute($sc_itransact_verify_message);
$my_desc=&script_and_substitute($sc_itransact_order_desc);
...
if ($sc_use_alternate_iTransact_submit_page ne "Yes") {
############  Start of Standard submit page - Select from this line to for copy functions ############
#### can use in custom submit page.  Custom file must be named: iTransact_custom_processing_lib.pl
#### Custom function (aka sub routine) to replace the code below must be named: iTransact_custom_submit_page


if ($iTransact_enablebodytags eq "1"){
$tempbodytags = qq~<input type="hidden" name="bodytags" value="$iTransact_enablebodytags">
<INPUT type="hidden" name="background" value="$iTransact_backimage_URL">
<INPUT type="hidden" name="bgcolor" value="$iTransact_bodyback">
<INPUT type="hidden" name="fontcolor" value="$iTransact_fontcolor">
<INPUT type="hidden" name="link" value="$iTransact_linkcolor">
<INPUT type="hidden" name="alink" value="$iTransact_alinkcolor">
<INPUT type="hidden" name="vlink" value="$iTransact_vlinkcolor">
~;
}

if ($iTransact_Logo_URL ne ""){
$tempbodytags .= qq~
<INPUT type="hidden" name="mastimage" value="1">
<INPUT type="hidden" name="mast" value="$iTransact_Logo_URL">
~;
}
...
$tempbodytags
<input type="hidden" name="mertext" value="$iTransact_merch_message">
...
<INPUT type=\"HIDDEN\" name=\"passback\" value=\"iTransact\">
<INPUT TYPE=\"HIDDEN\" NAME=\"iTransact\" VALUE=\"iTransact\">
...
<INPUT TYPE=HIDDEN NAME=\"disable_cards\" VALUE=\"$iTransact_disable_cards\">
<INPUT TYPE=HIDDEN NAME=\"disable_paypal\" VALUE=\"$iTransact_disable_paypal\">
<INPUT TYPE=HIDDEN NAME=\"disable_checks\" VALUE=\"$iTransact_disable_checks\">
...
<p class="ac_checkout_top_msg">$iTransact_order_ok_final_msg_tbl</p>
...
&codehook("iTransact_custom_submit_page");
...
if ($sc_pgp_or_gpg =~ /GPG/i) { # verify with GPG
  ...
  if (($result =~ /Good Signature/i) &&
    ...
    $verification = "$result\n";
# Could not verify iTransact signature because:\n$result\n";
   }
 }
...
if ($sc_use_alternate_iTransact_order_processing ne "Yes") {
############  Start of Standard Processing Code - Select from this line to for copy functions ############
#### can use in custom processing file.  Custom file must be named: iTransact_custom_processing_lib.pl
#### Place iTransact_custom_processing_lib.pl in the library directory of your store
#### Custom function (aka sub routine)  to replace the code below must be named: iTransact_custom_processing_function

&SecureStoreHeader;

$text_of_cart .= "Order Date:    $orderDate\n";
$text_of_cart .= "Gateway:       iTransact\n\n";
$orderLoggingHash{'GatewayUsed'} = "iTransact";
...
$sc_itransact_order_desc .= " - " . $itrans{'p5'};
...
if ($sc_send_order_to_email =~ /yes/i) {
  &send_mail($temp_admin_email, $sc_order_email, $sc_itransact_order_desc, $text_of_cart);
 }
...
&codehook("iTransact_custom_processing_function");
