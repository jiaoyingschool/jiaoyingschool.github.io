...
$versions{'NiftyPay-order_lib.pl'} = "5.5.003";
$sc_use_secure_header_at_checkout = 'yes';
$sc_use_secure_footer_for_order_form = 'yes';

# discount mods applied June 17 2005

$sc_order_response_vars{"NiftyPay"}="NiftyPay";
$sc_use_secure_header_at_checkout = 'yes';
&add_codehook("printSubmitPage","print_NiftyPay_SubmitPage");
&add_codehook("set_form_required_fields","NiftyPay_fields");
&add_codehook("gateway_response","check_for_NiftyPay_response");
###############################################################################
sub NiftyPay_check_and_load {
local($myname)="NiftyPay";
...
###############################################################################
sub check_for_NiftyPay_response {
  if ($form_data{'NiftyPay'} eq 'NiftyPay') {
    $cart_id = $form_data{'p4'};
    &set_sc_cart_path;
    &load_order_lib;
    &codehook("NiftyPay_order");
    &process_NiftyPay_Order;
    &call_exit;
   }
 }
###############################################################################
...
&codehook("NiftyPay_fields_bottom");

}

###############################################################################
sub NiftyPay_order_form_prep { # load the customer info ...
  &NiftyPay_check_and_load;
  if ($sc_NiftyPay_form_prep == 0) {
    if (-f "$sc_verify_order_path"){
      &read_verify_file;  
     } else {
      &codehook("load_customer_info");
     }
    $sc_NiftyPay_form_prep = 1;
   }
  return "";
 }
###############################################################################
sub NiftyPay_table_setup {
#
# To use this, put this in the email_text in the manager for HTML:
#
#	<!--agorascript-pre
#	  return $NiftyPay_cart_table;
#	-->
#
 ...
 while (<CART>)
  {
   ...
   if ($result eq '') {
     ...
     $NiftyPay_prod_in_cart .= "  --PRODUCT INFORMATION--\n\n";
    }
   $result .= "<TR><TD>$quantity</TD><TD>$product_id</TD>\n";
   $result .= "<TD>$name</TD><TD>$cost</TD>";
   $result .= "</TR>\n";
   $NiftyPay_prod_in_cart .= &cart_textinfo(*my_cart_fields);
  } # End of while (<CART>)
 close (CART);
 if ($result ne '') {
...
&codehook("NiftyPay_table_setup_bottom");

 $NiftyPay_cart_table = $result;

}
###############################################################################

sub print_NiftyPay_SubmitPage
...
$my_submit=&script_and_substitute($sc_niftypay_submit);
$my_change=&script_and_substitute($sc_niftypay_change);
$my_message=&script_and_substitute($sc_niftypay_verify_message);
$my_desc=&script_and_substitute($sc_niftypay_order_desc);
&NiftyPay_table_setup;
$zemail_text = &script_and_substitute($email_text,"NiftyPay");
...
if ($NiftyPay_enablebodytags eq "1"){
$tempbodytags = qq~<input type="hidden" name="bodytags" value="$NiftyPay_enablebodytags">
<INPUT type="hidden" name="background" value="$NiftyPay_backimage_URL">
<INPUT type="hidden" name="bgcolor" value="$NiftyPay_bodyback">
<INPUT type="hidden" name="fontcolor" value="$NiftyPay_fontcolor">
<INPUT type="hidden" name="link" value="$NiftyPay_linkcolor">
<INPUT type="hidden" name="alink" value="$NiftyPay_alinkcolor">
<INPUT type="hidden" name="vlink" value="$NiftyPay_vlinkcolor">
~;
}

if ($NiftyPay_Logo_URL ne ""){
$tempbodytags .= qq~
<INPUT type="hidden" name="mastimage" value="1">
<INPUT type="hidden" name="mast" value="$NiftyPay_Logo_URL">
~;
}
...
$tempbodytags
<input type="hidden" name="mertext" value="$NiftyPay_merch_message">
...
<INPUT type=\"HIDDEN\" name=\"passback\" value=\"NiftyPay\">
<INPUT TYPE=\"HIDDEN\" NAME=\"NiftyPay\" VALUE=\"NiftyPay\">
...
<INPUT TYPE=HIDDEN NAME=\"disable_cards\" VALUE=\"$NiftyPay_disable_cards\">
<INPUT TYPE=HIDDEN NAME=\"disable_paypal\" VALUE=\"$NiftyPay_disable_paypal\">
<INPUT TYPE=HIDDEN NAME=\"disable_checks\" VALUE=\"$NiftyPay_disable_checks\">
...
<p class="ac_checkout_top_msg">$NiftyPay_order_ok_final_msg_tbl</p>
...
############  End of Submit Page Code - Select until this line for copy functions ############
 ...
 &codehook("NiftyPay_custom_submit_page");

} # end of alternate submit page

}
############################################################################################
sub process_NiftyPay_Order {
...
if ($sc_pgp_or_gpg =~ /GPG/i) { # verify with GPG
  ...
  if (($result =~ /Good Signature/i) &&
    ...
    $verification = "$result\n";
# Could not verify NiftyPay signature because:\n$result\n";
   }
 }
...
$text_of_cart .= "Gateway:       NiftyPay\n\n";
$orderLoggingHash{'GatewayUsed'} = "NiftyPay";
...
$sc_niftypay_order_desc .= " - " . $itrans{'p5'};
...
if ($sc_send_order_to_email =~ /yes/i) {
  &send_mail($temp_admin_email, $sc_order_email, $sc_niftypay_order_desc, $text_of_cart);
 }
...
############  End of Standard Processing Code - Select until this line for copy functions ############
 ...
 &codehook("NiftyPay_custom_processing_function");
