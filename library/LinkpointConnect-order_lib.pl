...
##############################################################################
$versions{'LinkpointConnect-order_lib.pl'} = "5.5.000";
$sc_use_secure_header_at_checkout = 'yes';
$sc_use_secure_footer_for_order_form = 'yes';
$sc_LinkpointConnect_form_prep = 0;
&add_codehook("printSubmitPage","print_LinkpointConnect_SubmitPage");
&add_codehook("set_form_required_fields","LinkpointConnect_fields");
$sc_order_response_vars{"LinkpointConnect"}="oid";
&add_codehook("gateway_response","check_for_LinkpointConnect_response");
###############################################################################
sub LinkpointConnect_check_and_load {
local($myname)="LinkpointConnect";
...
###############################################################################
sub check_for_LinkpointConnect_response {
  if ($form_data{'oid'}) {
    ($cart_id,$lp_inv) = split(/\|/,$form_data{'oid'},2);
    &set_sc_cart_path;
    &load_order_lib;
    &codehook("LinkpointConnect_order");
    &process_LinkpointConnect_order;
    &call_exit;
   }
 }
###############################################################################
sub LinkpointConnect_order_form_prep { # load the customer info ...
  &LinkpointConnect_check_and_load;
  if ($sc_LinkpointConnect_form_prep == 0) {
    if (-f "$sc_verify_order_path"){
      &read_verify_file;  
     } else {
      &codehook("load_customer_info");
     }
    $sc_LinkpointConnect_form_prep = 1;
   }
  return "";
 }

###############################################################################
sub LinkpointConnect_fields{
local($myname)="LinkpointConnect";
...
# propagated from offline gateway 2006-4-5
...
&codehook("LinkpointConnect_fields_bottom");

}

###############################################################################
sub LinkpointConnect_verification_table {
  local ($rslt)="";
...
&codehook("LinkpointConnect_verification_table_bottom");

  return $rslt;
 }
###############################################################################

sub print_LinkpointConnect_SubmitPage
...
$my_submit=&script_and_substitute($sc_linkpoint_submit);
$my_change=&script_and_substitute($sc_linkpoint_change);
$my_message=&script_and_substitute($sc_linkpoint_verify_message);
$my_desc=&script_and_substitute($sc_linkpoint_order_desc);

$my_oid = $cart_id . "|" . $invoice_number;
$mytable = &LinkpointConnect_verification_table;
&read_verify_file;
...
if ($sc_use_alternate_LinkpointConnect_submit_page ne "Yes") {
############  Start of Standard submit page - Select from this line to for copy functions ############
#### can use in custom submit page.  Custom file must be named: LinkpointConnect_custom_processing_lib.pl
#### Custom function (aka sub routine) to replace the code below must be named: LinkpointConnect_custom_submit_page
...
if ($sc_linkpoint_auth_only =~ /yes/i) {
print <<ENDOFTEXT;
...
&codehook("LinkpointConnect_custom_submit_page");
...
sub process_LinkpointConnect_order {
...
if ($sc_use_alternate_LinkpointConnect_order_processing ne "Yes") {
############  Start of Standard Processing Code - Select from this line to for copy functions ############
#### can use in custom processing file.  Custom file must be named: LinkpointConnect_custom_processing_lib.pl
#### Place LinkpointConnect_custom_processing_lib.pl in the library directory of your store
#### Custom function (aka sub routine)  to replace the code below must be named: LinkpointConnect_custom_processing_function

&SecureStoreHeader;

$text_of_cart .= "Order Date:    $orderDate\n";
$text_of_cart .= "Gateway:       LinkpointConnect\n\n";
$orderLoggingHash{'GatewayUsed'} = "LinkpointConnect";
...
$orderLoggingHash{'invoiceNumber'} = "$lp_inv";
...
if ($sc_send_order_to_email =~ /yes/i) {
  &send_mail($temp_admin_email, $sc_order_email, "AgoraCart - LinkpointConnect Order - $lp_inv",$text_of_cart);
 }
...
############  End of Standard Processing Code - Select until this line for copy functions ############
 ...
 &codehook("LinkpointConnect_custom_processing_function");

} # end of alternate processing

} # End of process_LinkpointConnect_order
