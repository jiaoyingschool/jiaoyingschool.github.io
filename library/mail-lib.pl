...
# Date Last Modified: Sept 4, 2008 by Mister Ed

$versions{'mail-lib.pl'} = "5.5.000";
...
local $sc_replyto_email_temp = "$sc_replyto_email";
...
# added by Mister Ed September 4, 2008
if ($sc_replyto_email_temp eq '') {
    $sc_replyto_email_temp = "$sc_admin_email";
}
if ($sc_email_content_type eq '') {
    $sc_email_content_type = "text/plain";
}
...
#
# added Reply-to, x-mailer, content type, and content transfer fields
# by Mister Ed September 4, 2008

    print MAIL <<__END_OF_MAIL__;
To: $touser
From: $fromuser
Reply-to: $sc_replyto_email_temp
Subject: $subject
X-Mailer: AgoraCart $versions{'agora.cgi'}
Content-Type: $sc_email_content_type;
